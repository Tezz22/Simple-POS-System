<template>
  ...
  >
    ...
    >
      <h3 class="text-sm font-black tracking-wider text-gray-900 dark:text-white uppercase flex items-center justify-center gap-1.5">
        <Icon icon="heroicons:building-storefront-solid" class="w-4 h-4" />
        TOKO KITA
      </h3>
    ...
    <div class="py-3 space-y-2 border-b border-dashed border-gray-200 dark:border-gray-800">
      <div v-for="item in items" :key="item.id" class="space-y-0.5">
        <div class="flex justify-between font-bold text-gray-900 dark:text-white">
          ...
          <!-- FIX: Hapus teks literal "FormatRp", ganti menjadi "Rp" -->
          <span>Rp {{ formatUang(item.price * item.qty) }}</span>
        </div>
        <div class="text-[10px] text-gray-400">
          {{ item.qty }} x Rp {{ formatUang(item.price) }}
        </div>
      </div>
    ...
    <div class="py-3 space-y-1.5 text-[11px]">
      <div class="flex justify-between font-black text-xs text-gray-900 dark:text-white pt-1">
        ...
        <span>Rp {{ formatUang(transaction.total) }}</span>
      </div>
    </div>
