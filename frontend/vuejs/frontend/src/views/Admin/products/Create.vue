  ...
  <div class="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-200">
    <div class="px-4 sm:px-6 lg:px-8 py-8 max-w-3xl mx-auto space-y-6">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div class="space-y-1">
          <h1 class="text-3xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
            <Icon icon="heroicons:plus-circle-solid" class="w-8 h-8 text-teal-600 dark:text-teal-400" />
            Tambah Produk Baru
          </h1>
          <p class="text-sm text-gray-600 dark:text-gray-400">
            Pastikan seluruh data kode unik dan kalkulasi harga pokok diisi dengan benar.
          </p>
        </div>
      </div>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
        <!-- Info Alert -->
        <div class="bg-green-50 dark:bg-green-950/20 border-b border-green-200 dark:border-green-900/30 p-4 flex items-start gap-3">
          <Icon icon="heroicons:check-circle-solid" class="w-5 h-5 text-green-600 dark:text-green-400 flex-shrink-0 mt-0.5" />
          <div>
            <p class="text-sm font-medium text-green-900 dark:text-green-200">Tips Penting</p>
            <p class="text-sm text-green-800 dark:text-green-300">Kode produk harus unik. Harga jual harus lebih tinggi dari harga beli pokok.</p>
          </div>
        </div>
          ...
          <form @submit.prevent="handleSubmit" class="space-y-6">
            ...
            <div class="space-y-4">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <FormGroup label="Kode Produk / SKU" required :error="errors?.code">
                  <BaseInput v-model="form.code" placeholder="Contoh: SKU-1002" />
                </FormGroup>

                <FormGroup label="Barcode (Opsional)" :error="errors?.barcode">
                  <BaseInput v-model="form.barcode" placeholder="Scan nomor barcode" />
                </FormGroup>
              </div>
            </div>
            ...
            <div class="space-y-4">
              ...
              <FormGroup label="Nama Lengkap Barang" required :error="errors?.name">
                <BaseInput v-model="form.name" placeholder="Masukkan nama komoditas produk" />
              </FormGroup>
            ...
            <div class="space-y-4">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <FormGroup label="Harga Beli Pokok (Rp)" required :error="errors?.purchase_price">
                  <BaseInput v-model.number="form.purchase_price" type="number" min="0" />
                </FormGroup>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <FormGroup label="Stok Jumlah Awal" required :error="errors?.stock">
                  <BaseInput v-model.number="form.stock" type="number" min="0" />
                </FormGroup>

                <FormGroup label="Batas Minimum Stok" required :error="errors?.min_stock">
                  <BaseInput v-model.number="form.min_stock" type="number" min="0" />
                </FormGroup>
              </div>
            </div>
