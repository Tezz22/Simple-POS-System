  ...
  <div class="p-6">
    ...
    <div>
      <h1 class="text-xl font-black text-text-primary dark:text-white tracking-tight flex items-center gap-2">
        <Icon icon="heroicons:shopping-cart-solid" class="w-6 h-6 text-primary dark:text-primary-light" />
        Kasir / Point of Sale
      </h1>
        ...
        <SectionCard>
          <template #header>
            <div class="flex items-center justify-between w-full">
              <h3 class="text-xs font-black uppercase tracking-wider text-text-secondary dark:text-gray-400 flex items-center gap-1.5">
          ...
          <div v-if="cart.length === 0" class="py-8 text-center">
            <Icon icon="heroicons:shopping-cart" class="w-10 h-10 text-gray-300 dark:text-gray-600 mx-auto mb-2" />
    ...
    <BaseModal :show="successModal.show" title="Transaksi Berhasil" @close="closeSuccessModal">
      <div class="flex flex-col items-center text-center gap-3 py-2">
        <div class="w-14 h-14 rounded-full bg-green-50 dark:bg-green-950/30 flex items-center justify-center">
        ...
        <div class="w-full bg-gray-50 dark:bg-gray-900/50 rounded-xl p-3 space-y-1.5 text-xs">
          <div class="flex justify-between">
            ...
            <span class="font-bold text-text-primary dark:text-white">{{ formatRupiah(successModal.total) }}</span>
          </div>
          <div class="flex justify-between">
            <span class="text-text-secondary dark:text-gray-400">Uang Diterima</span>
            <span class="font-bold text-text-primary dark:text-white">{{ formatRupiah(successModal.cash) }}</span>
          </div>
          <div class="flex justify-between border-t border-dashed border-gray-200 dark:border-gray-700 pt-1.5">
...
const increaseQty = (id) => {
  transactionStore.increaseQty(id)
}
...
const handlePayment = async (payment) => {
  try {
    ...
    transactionStore.clearCart()
    productStore.fetchAll() // refresh stok produk terbaru
  } catch (error) {
    console.error(error)
    toastStore.show?.({
      type: 'error',
